...
"Player" : {
        ...
        "Inventory" : {
                "Нож" : {"Урон": "2-4 урона", "Скорость" : "4f" , "scale" : "AC", "Equipment" : "Enable"},
                "Кожаная куртка" : {"Защита" : "2 Зщт", "Equipment" : "Enable"},
...
"Player" : {
        ...
        "Inventory" : {"Кожаная куртка" : {"Защита" : "2 Зщт", "Equipment" : "Enable"},},
...
def Fight(player1, player2):
        ...
        def Enemy_move(PDamage, PDefens, EDefens):
                ...
                for val in player2["Player"]["Inventory"]:
                        if player2["Player"]["Inventory"][val].get("Equipment") == "Enable":
                                if player2["Player"]["Inventory"][val].get("Урон") != None:
                                        EDamage = player2["Player"]["Statistics"]["Stats"][1] + random.randint(int(player2["Player"]["Inventory"][val].get("Урон")[0]), int(player2["Player"]["Inventory"][val].get("Урон")[2]))
                                else:
        ...
        def Get_Damage_and_Defens(param):
                if param == "PDmg":
                        for val in player1["Player"]["Inventory"]:
                                if player1["Player"]["Inventory"][val].get("Equipment") == "Enable":
                                        if player1["Player"]["Inventory"][val].get("Урон", None) != None:
                                                return int((int(player1["Player"]["Inventory"][val].get("Урон")[0]) + int(player1["Player"]["Inventory"][val].get("Урон")[2]))/2)
                if param == "PDef":                                                
                        for val in player1["Player"]["Inventory"]:
                                if player1["Player"]["Inventory"][val].get("Equipment") == "Enable":
                                        if player1["Player"]["Inventory"][val].get("Защита") != None:
                                                return int(player1["Player"]["Inventory"][val].get("Защита")[0])

                if param == "EDmg":
                        for val in player2["Player"]["Inventory"]:
                                if player2["Player"]["Inventory"][val].get("Equipment") == "Enable":
                                        if player2["Player"]["Inventory"][val].get("Урон", None) != None:
                                                return int((int(player2["Player"]["Inventory"][val].get("Урон")[0]) + int(player2["Player"]["Inventory"][val].get("Урон")[2]))/2)
                if param == "EDef":                                                
                        for val in player2["Player"]["Inventory"]:
                                if player2["Player"]["Inventory"][val].get("Equipment") == "Enable":
                                        if player2["Player"]["Inventory"][val].get("Защита") != None:
                                                return int(player2["Player"]["Inventory"][val].get("Защита")[0]) 
...
""")
                                ...
                                # Игрок не успевает и получат урон и минус по общим фреймам
                                        for val in player1["Player"]["Inventory"]:
                                                if player1["Player"]["Inventory"][val].get("Equipment") == "Enable":
                                                        if player1["Player"]["Inventory"][val].get("Урон") != None:
                                                                PDamage = player1["Player"]["Statistics"]["Stats"][1] + random.randint(int(player1["Player"]["Inventory"][val].get("Урон")[0]), int(player1["Player"]["Inventory"][val].get("Урон")[2]))
                                        Hit = PDamage
                                        ...
                                        print("Equipment: ")
                                        for key, value in zip(objects, feature):
                                                if value.get("Equipment") == "Enable":
                                                        print(f"""{key} | {value.get("Урон", "")} {value.get("Defense", "")}""")
                                                
                                        print("Else: ")
                                        for key, value in zip(objects, feature):
                                                if value.get("Equipment", "Disabled") == "Disabled":
                                                        print(f"""{key} | {value.get("Урон", "")} {value.get("Defense", "")}""")
